function imgVer(Config) {
	...
	var left_Num = -X + 10;
	var html = '<div style="position:relative;margin:10px 10px 0">';
	...
	html += '</div>';
	html += '<div style="position:relative;width:' + w + 'px;margin:10px;">';
	html += '<div style="border:1px solid #c3c3c3;border-radius:24px;background:#dfe1e2;">';
	html += '<p style="font-size:12px;color: #a1aab1;line-height:28px;margin:0;text-align:right;padding-right:22px;">按住左边滑块，拖动完成上方拼图</p>';
	html += '</div>';
	html += '<div class="slider-btn"></div>';
	
	html += '</div>';
	html += '<div style="border-top: 1px solid #f5f5f5;height: 36px;">';
	html += '<div class="re-btn"><em class="close-ver"></em><a></a></div>';
	html += '</div>';
	el.html(html);
	var d = PL_Size / 3;
	...
	onmouseup = function(e) {
		...
		if(moveStart == '') {} else {
			if(Max_left > moveEnd_X && moveEnd_X > Min_left) {
				$(".ver-tips").html('<i style="background-position:-4px -88px;"></i><span style="color:#42ca6b;">验证通过</span><span></span>');
				...
				$(".ver-tips").html('<i style="background-position:-4px -108px;"></i><span style="color:red;">验证失败:</span><span style="margin-left:4px;">拖动滑块将悬浮图像正确拼合</span>');
				$(".ver-tips").addClass("slider-tips");
				setTimeout(function() {
					$(".ver-tips").removeClass("slider-tips");
				}, 2000);
