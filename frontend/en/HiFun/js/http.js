...
//		myFriUrl: "http://is-api.kaixin001.com/invite/incomeLog/token="
	...
	G.getHead = function(token, people) {
		G.req({
			...
			success: function(resJson) {
				var res = JSON.parse(resJson);
				console.log("report success!", people);
				if(res.code == 200 && res.msg) {
					...
					if(people == 1) {
						...
						/*txt.style.marginLeft = txt.style.marginRight = "33%";*/
					}
				}
			},
			error: function() {
				...
				if(res.code == 200 && res.msg) {
					for(var i = 0; i < res.msg.length; i++) {
					  ...
					  if(res.msg[i].nickname){
						...
						var yuan = document.createTextNode('Token');
						var canget = document.createTextNode('30Token');

						var img = document.createTextNode('The reward has been collected');
